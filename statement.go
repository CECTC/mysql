...
func (stmt *mysqlStmt) Exec(args []driver.Value) (driver.Result, error) {
	...
	if stmt.mc.ctx != nil {
		...
		if isDelete {
			...
			if err := stmt.mc.writeCommandPacketUint32(comStmtClose, stmt.id); err != nil {
				log.Error(err)
			}
...
func (stmt *mysqlStmt) Query(args []driver.Value) (driver.Rows, error) {
	if stmt.mc.ctx != nil {
		p := parser.New()
		act, _ := p.ParseOneStmt(stmt.sql, "", "")
		selectForUpdateStmt, ok := act.(*ast.SelectStmt)
		if ok && selectForUpdateStmt.LockInfo.LockType == ast.SelectLockForUpdate {
